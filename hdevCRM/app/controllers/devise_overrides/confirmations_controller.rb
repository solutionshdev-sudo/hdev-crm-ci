class DeviseOverrides::ConfirmationsController < Devise::ConfirmationsController
  ...
  def render_confirmation_error
    if @confirmable.blank?
      ...
      render json: { message: I18n.t('errors.api.devise.already_confirmed'), redirect_url: '/' }, status: :unprocessable_entity
    else
      render json: { message: I18n.t('errors.api.devise.failure'), redirect_url: '/' }, status: :unprocessable_entity
    end
  end
