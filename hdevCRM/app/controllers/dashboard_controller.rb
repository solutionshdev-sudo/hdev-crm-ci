class DashboardController < ActionController::Base
  ...
  before_action :resolve_agency_from_host
  ...
  # login happens, so branding can be resolved for SSR and window.globalConfig.
  def resolve_agency_from_host
    domain = request.host
    return if domain.blank? || domain == URI.parse(ENV.fetch('FRONTEND_URL', '')).host

    @agency = Agency.resolve_by_host(domain)
  end

  def set_global_config
    @global_config = GlobalConfig.get(*GLOBAL_CONFIG_KEYS).merge(app_config)
    @global_config = @agency.apply_branding(@global_config) if @agency.present?
  end
  ...
  def render_hc_if_custom_domain
    # an agency dashboard domain takes precedence over a help center domain
  ...
  def app_config
    {
      APP_VERSION: HdevCrm.config[:version],
      ...
      IS_ENTERPRISE: HdevApp.enterprise?,
