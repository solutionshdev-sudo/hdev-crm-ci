...
RSpec.describe Captain::BaseTaskService do
  ...
  describe 'error handling' do
    ...
    before do
      ...
      allow(HdevExceptionTracker).to receive(:new).with(error, account: account).and_return(exception_tracker)
      allow(exception_tracker).to receive(:capture_exception)
    end

    it 'tracks exceptions' do
      expect(HdevExceptionTracker).to receive(:new).with(error, account: account).and_return(exception_tracker)
