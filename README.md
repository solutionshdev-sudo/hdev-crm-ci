...
Você acaba de instalar o HDEV. Em alguns minutos, sua empresa vai
ter uma memória própria, uma identidade visual aplicada em tudo que
o sistema gerar, e um arsenal de skills prontas pra fazer marketing,
SEO, ads e operação rodarem com você dirigindo.
...
```
Clona o https://github.com/solutionshdev-sudo/hdev-template.git na pasta atual,
...
```
git clone https://github.com/solutionshdev-sudo/hdev-template.git
cd hdev-template
code .
```
...
Quando o `/instalar` terminar, renomeia a pasta `HDEV/` pro nome do teu
negócio (fecha o VS Code, renomeia no Explorer/Finder, abre de novo). A
pasta não fica como "HDEV" — ela é o teu negócio agora.
...
em skill personalizada · `/atualizar-sistema` puxa melhorias novas do
template pra esse projeto.
...
closed loop dentro do HDEV (decide → executa → captura → realimenta →
ajusta sozinho).
...
[github.com/solutionshdev-sudo](https://github.com/solutionshdev-sudo)
