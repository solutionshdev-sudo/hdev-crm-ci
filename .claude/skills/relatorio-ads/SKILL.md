...
### Passo 0 — Checar se tem MCP do Meta Ads conectado

Se as ferramentas do Meta Ads MCP estiverem disponíveis na sessão (nomes começando com `mcp__` e contendo `meta_ads`), **não pedir CSV da Meta** — puxar direto da API:

- Contas e campanhas ativas, gasto, impressões, cliques, resultados do período
- Tendência de performance e sinais de anomalia (ferramentas de insights do MCP)
- Benchmarks do setor quando disponíveis

Google Ads continua via CSV exportado (sem MCP). Se não houver MCP conectado, seguir o fluxo de CSV pros dois canais.
