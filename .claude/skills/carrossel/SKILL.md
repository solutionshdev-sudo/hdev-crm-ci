...
- **Playwright:** pra renderizar HTML em PNG via `scripts/render-carrossel.js`
...
2. Renderizar com o script fixo do template (não criar `render.js` por pasta):
```bash
node scripts/render-carrossel.js marketing/conteudo/<pasta>
# formato Reels/TikTok, quando pedido:
node scripts/render-carrossel.js marketing/conteudo/<pasta> --out tiktok --size 1080x1920
```
   Requer Playwright instalado na raiz (`npm install` — já está no `package.json`; depois `npx playwright install chromium`, uma vez só).
...
- HTMLs: um único arquivo `carrossel.html` com todos os slides. Inline CSS
- Render: sempre via `scripts/render-carrossel.js` (script fixo do template — não gerar `render.js` por pasta)
