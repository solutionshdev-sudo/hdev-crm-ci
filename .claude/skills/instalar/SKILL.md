...
name: instalar
description: >
  Instala o HDEV no negócio do usuário. Entrevista sobre empresa, tom de voz,
  ...
  pedir explicitamente "rodar /instalar", "instalar o HDEV", "primeiro setup".
---

# /instalar — Instalação inicial do HDEV
...
### 0. Perfil global (evita repetir a entrevista a cada projeto)

Conferir se existe `~/.hdev/perfil.md`. Se existir, ler e mostrar um resumo:

> "Achei teu perfil global (tom de voz, preferências, dados pessoais) de instalações anteriores. Uso ele aqui e só pergunto o que é específico desse projeto?"

Se o usuário aceitar, **pular as perguntas 5-6 da Fase 2** (voz) e preencher `_memoria/preferencias.md` a partir do perfil global. As perguntas de negócio e foco (1-4, 7-8) continuam — mudam a cada projeto.

### 0.5. Diagnóstico de ambiente

Rodar rápido (sem alarde, uma linha de resultado no final do setup):

- `git --version` e `git config user.name` / `user.email` — se não configurado, perguntar e configurar
- `node --version` — precisa 20+ pros scripts de `scripts/`
- `gh --version` — opcional, facilita o `/salvar` criar repositório
- `npx playwright --version` — opcional, necessário só pro render de carrossel

Registrar o resultado na linha **Ferramentas** de `_memoria/empresa.md` (ex: "git ✓, node 22 ✓, gh ✓, playwright ✗ — instalar quando for usar /carrossel"). O que estiver faltando não trava o setup — as skills avisam quando precisarem.

### 1. Nome da pasta

Conferir o nome da pasta atual (`basename "$(pwd)"`). Se for `hdev`, `HDEV`, `HDEV-main`, `hdev-main` ou variação genérica:

> "Notei que a pasta atual ainda tem nome genérico ('<nome-atual>'). O ideal é a pasta ter o nome do seu negócio, não 'HDEV'. Quando terminarmos o setup, te lembro de renomear (é rápido — fechar VS Code, renomear a pasta no Finder/Explorer, abrir de novo). Bora seguir?"
...
**Obrigatório:** o CLAUDE.md final PRECISA manter a seção "Regras de operação do sistema" com a linha `@_sistema/regras.md` (todos os moldes já trazem). É ela que importa o núcleo do sistema — sem ela, o auto-aprendizado e o fluxo de skills param de funcionar. Conferir depois de escrever.
...
✓ CLAUDE.md adaptado pro perfil [perfil] (regras do sistema preservadas)
✓ Ambiente: [resultado do diagnóstico em uma linha]
```

Se `~/.hdev/perfil.md` NÃO existia, oferecer:

> "Quer que eu salve teu tom de voz e preferências pessoais em `~/.hdev/perfil.md`? No próximo projeto que você criar com o template, o setup pula essas perguntas."

Se aceitar, criar o arquivo com: nome, tom de voz (da pergunta 5), ranços de escrita (pergunta 6), e ferramentas do diagnóstico.
...
Por fim, oferecer o primeiro commit como baseline:

> "Recomendo salvar esse estado inicial no GitHub agora (`/salvar`) — assim tudo que a gente produzir daqui pra frente fica versionado desde o dia 1."

Se aceitar, rodar o fluxo do `/salvar` (que já cria o repositório se não existir e aplica a trava de segredos).
