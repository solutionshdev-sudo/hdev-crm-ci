...
- Modelo de negócio inicial: white-label pra agências. **Estrutura de planos implementada (06/08):** `Plan` com limites reais + `Plan::LimitEnforcer` + CRUD em `/super_admin/plans`; Stripe em código (webhook idempotente, checkout, portal — F7) com quota de IA por contador atômico (F7.5). A conta Stripe real liga depois da F8.
